        ...
        <div class="row g-3">
          <div class="shop-toolbar pb-2">
            ...
            <div class="column">
              <div class="shop-view"><a class="list-view {{Session::has('view_catalog') && Session::get('view_catalog') == 'grid' ? 'active' : ''}} " data-step="grid" href="javascript:;" data-href="{{route('front.catalog').'?view_check=grid'}}"><i class="fas fa-th-large"></i></a>
                <a class="list-view {{Session::has('view_catalog') && Session::get('view_catalog') == 'list' ? 'active' : ''}}" href="javascript:;" data-step="list" data-href="{{route('front.catalog').'?view_check=list'}}"><i class="fas fa-list"></i></a></div>
            </div>
          </div>
          <!-- Products Grid-->
          ...
          <div class="col-lg-3 order-lg-1">
            ...
            <aside class="sidebar sidebar-offcanvas position-left"><span class="sidebar-close"><i class="icon-x"></i></span>
              ...
              <!-- Widget Quick Filter-->
              <section class="widget widget-categories card rounded p-4">
               <h3 class="widget-title">{{__('Quick filter')}}</h3>
                  <ul id="quick_filter">
                    <li><a datahref="">{{__('All products')}}</a><span>({{$all_products}})</span></li>
                    <li class=""><a href="javascript:;" data-href="feature">{{__('Featured products')}}</a><span>({{$feature_products}})</span></li>
                    <li class=""><a href="javascript:;" data-href="best">{{__('Best sellers')}}</a><span>({{$best_products}})</span></li>
                    <li class=""><a href="javascript:;" data-href="top">{{__('Top rated')}}</a><span>({{$top_products}})</span></li>
                  </ul>
              </section>

              <!-- Widget Price Range-->
              <section class="widget widget-categories card rounded p-4">
                <h3 class="widget-title">{{__('Price Range')}}</h3>
              ...
              <section class="widget widget-categories card rounded p-4">
                <h3 class="widget-title">{{__('Item Attributes')}}</h3>
                <ul>
                  <li class="has-children">
                    <a class="empty_attrubute" href="javascript:;" data-href="">{{__('All Attributes')}}</a>
                  </li>
                    @foreach ($attrubutes as $attrubute)
                    <li class="has-children  {{isset($category) && $category->id == $attrubute->id ? 'expanded active' : ''}} ">
                      <a class="attribute_search" href="javascript:;"  data-href="{{$attrubute->name}}">{{$attrubute->name}}</a>
                        <ul id="option_search">
                            @foreach ($attrubute->options as $options)
                            <li class="{{isset($subcategory) && $subcategory->id == $options->id ? 'active' : ''}}">
                              <a class="option" href="javascript:;" data-href="{{$options->name}}">{{$options->name}}</a>
                            </li>
                            @endforeach
                        </ul>
                      </li>
              ...
              <section class="widget widget-categories card rounded p-4">
                ...
                <div class="custom-control custom-checkbox">
                    ...
                    <label class="custom-control-label" for="{{$getbrand->slug}}">{{$getbrand->name}}<span class="text-muted">({{$getbrand->items_count}})</span></label>
                  </div>
                @endforeach
              </section>
