...
$(document).on('click','.wishlist_store',function(e){
    ...
    $.get(wishlist_url,function(response){
        ...
        function getData(status = 0, check = 0, item_key = 0, qty = 0) {
            let itemId;
            let type;
            if (check != 0) {
                itemId = check;
            ...
            if (status == 1) {
                let addToCartUrl = `${mainurl}/product/add/cart?item_id=${itemId}&options_ids=${options_ids}&attribute_ids=${attribute_ids}&quantity=${quantity}&type=${type}&item_key=${item_key}`;
                $.ajax({
                    type: "GET",
                    url: addToCartUrl,
                    success: function (data) {
